function    [cs,cs_sigma,int_year,int_month,time] = gmt_readgsm_GRACE_JPL_RL06(dir_in,file_name,lmax)
%   修改gmt_readgsm，用于读取GRACE（GSM）类型的JPL RL06数据
% INPUT:
%   dir_in      full path
%   file_name   filename 
...
while size(tline,2)<6 || ~strcmp(tline(1:6),'GRCOF2')
    ...
    if size(tline,2)>3 && strcmp(tline(5:10),'degree')  % In JPL GSM files, the maximum degree changes, 60 or 90
        degree_max=str2double(tline(29:30));
        break;
    end
end
fclose(fid);
% re-read the file, skip the comment lines
% 'headlines'属性指明跳过几行，RL06数据跳过134行头文件数据
