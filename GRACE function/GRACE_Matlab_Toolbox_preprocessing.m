...
%第159行处理RL05格式数据，在处理GSM_CSR_RL06数据时替换成160行
%为了添加GRACE和GRACE-FO之间缺失的数据，故采用平均值法填补，具体实现方法位于301-303行（额外添加）,并扩大其他cs矩阵的数量
%第283行GIA文件路径有所修改
%采用三次样条插值填补缺失数据，位于204-205行
clc;clear;
addpath('G:\fengweiigg-GRACE_Matlab_Toolbox-61c8064\GRACE_data\LoveNumber_files');
controlfile_path='G:\GRACE_processing\GRAMAT_Control_File_jpl_RL06_swenson_02_17.txt';
dir_out='G:\GRACE_processing\hbpy2002_2017\';%替换第65行，输出文件夹
...
%         [cs(ii,:,:),cs_sgi(ii,:,:),int_year(ii),int_month(ii),time(ii)] = gmt_readgsm_GRACE_CSR_RL06(dir_in,file_name{ii},lmax);
        [cs(ii,:,:),cs_sgi(ii,:,:),int_year(ii),int_month(ii),time(ii)] = gmt_readgsm_GRACE_JPL_RL06(dir_in,file_name{ii},lmax);
    end
end
...
% [ cs_replace,cs_sgi,int_year,int_month,time] = GRACE_Spineinterp(cs_replace,cs_sgi,int_year,int_month,time,lmax);
% num_file=num_file+11;
% ----------------------------------------
% Remove average
% ----------------------------------------
...
if strcmp(option_gia,'GIA_Removed_Geru')
    ...
    grid_pgr  = gmt_readpgr('G:\fengweiigg-GRACE_Matlab_Toolbox-61c8064\GRACE_data\GIA_files\GIA_n100_mass_0km.txt'); 
...
% %数据处理后，填补2017.07-2018.05之间缺失的月份
% [ cs_fltr,cs_sgi,int_year,int_month,time] = GRACE_FillMonth(cs_fltr,cs_sgi,int_year,int_month,time,lmax);
%  num_file=num_file+11;
